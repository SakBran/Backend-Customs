...
const RejectReport: React.FC = () => {
    ...
    const dataToExport = Array.from(
      ...
      return {
        ...
        'Application No': cells[1].innerText,
          ...
          'Reject Message': cells[3].innerText,
          'Reject Date': cells[4].innerText,
          'Business Name': cells[5].innerText,
          'Business Name Myanmar': cells[6].innerText,
          'Company Reg No': cells[7].innerText,
          'Sme Reg No': cells[8].innerText,
          'Eir Reg No': cells[9].innerText,
          'Apply Type': cells[10].innerText,
      };
    });
  ...
  const normalizeData = (data: AnyObject[]) => {
    return data.map((item) => ({
      ...
      rejectMessage: item.rejectMessage || null,
      rejectDate: item.rejectDate ? formatDate(item.rejectDate) : null,
  ...
  const fetch = async (url: string): Promise<PaginationType> => {
    try {
      ...
      responseData.data.forEach((data) => {
       ...
       // data['rejectDate'] = formatDate(data['rejectDate']);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await fetch('Report/RejectReport');
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };
