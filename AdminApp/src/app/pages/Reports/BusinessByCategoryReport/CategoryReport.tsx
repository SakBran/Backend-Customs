...
import { GetCategoryReportData } from './APIAction';
...
export const CategoryReport = () => {
  const [dataList, setDataList] = useState<CategoryReportDTO[]>([]);
  ...
  const onFinish = (values: any) => {
    ...
    //modified
    // navigate(`?startDate=${values['startDate']}&endDate=${values['endDate']}`);

    const APIAction = async () => {
      try {
        const result = await GetCategoryReportData(
          values['startDate'],
          values['endDate'],
          values['applyType']
        );
        if (result) {
          console.log(result);
          const temp: CategoryReportDTO[] = JSON.parse(JSON.stringify(result));
          setDataList(temp);
          setLoading(false);
        }
      } catch (ex) {
      ...
      <div style={{ width: '100%', justifyContent: 'center' }}>
        <Spin tip="Fetching from database..." spinning={loading}>
          ...
          >
            ...
            <tbody>
              {dataList.map((data: CategoryReportDTO, index: number) => (
                <tr key={index}>
                  <td>{index + 1}</td>
                  {/* <td>{data.category}</td> */}
                  <td>{data.category}</td>
